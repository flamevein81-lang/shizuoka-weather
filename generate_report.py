...
<body>
  ...
  <script>
    (function () {{
      ...
      /* ── passive イベントのサポート確認（古いAndroid対応） ── */
      var passiveOpt = false;
      try {{
        var testOpts = Object.defineProperty({{}}, 'passive', {{
          get: function () {{ passiveOpt = {{ passive: true }}; }}
        }});
        window.addEventListener('_', null, testOpts);
        window.removeEventListener('_', null, testOpts);
      }} catch (e) {{}}
      ...
      /* ── イベント登録（PC + iOS + Android Chrome すべて対応） ── */
      /* document に対して登録すると Android Chrome で確実に捕捉できる */
      document.addEventListener('scroll',    onScroll, passiveOpt);
      window.addEventListener('scroll',      onScroll, passiveOpt);
      window.addEventListener('touchmove',   onScroll, passiveOpt);
      ...
      /* ── ヘッダー高さを CSS変数・インラインスタイル両方にセット ── */
      function updateSize() {{
        var h = header.offsetHeight;
        document.documentElement.style.setProperty('--header-h', h + 'px');
        body.style.paddingTop = h + 'px';
        lastY = getScrollY();   /* リサイズ後に lastY をリセット */
      }}

      window.addEventListener('resize', updateSize, passiveOpt);

      /* 初期化：DOMContentLoaded / load / 遅延 の3重保険 */
